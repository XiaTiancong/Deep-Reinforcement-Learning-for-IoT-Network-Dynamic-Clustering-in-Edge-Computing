...
class Cluster(object,):
        ...
        if flag_benchmark_topology == 1:
            ...
            with open('state_xcor_20.txt', 'w') as f:
                for item in self.state_xcor:
                    f.write("%s\n" % item)
            f.close()
            with open('state_ycor_20.txt', 'w') as f:
    ...
    def act_move_events(self, total_tick):
        if read_AGV:
            ...
            # Here is new: Make every turn random
            if self.state_event_xcor < 0 or self.state_event_xcor > deploy_range_x or self.state_event_ycor < 0 or self.state_event_ycor > deploy_range_y:
                if self.state_event_xcor <= 0 and self.state_event_step_rate_xcor <= 0:
                    new_direction = random.uniform(-math.pi/2, math.pi/2)
                    self.state_event_step_rate_xcor = math.cos(new_direction)
                    self.state_event_step_rate_xcor = math.sin(new_direction)
                elif self.state_event_ycor <= 0 and self.state_event_step_rate_xcor <= 0:
                    new_direction = random.uniform( 0, math.pi)
                    self.state_event_step_rate_xcor = math.cos(new_direction)
                    self.state_event_step_rate_xcor = math.sin(new_direction)
                elif self.state_event_xcor >= deploy_range_x and self.state_event_step_rate_xcor >= 0:
                    new_direction = random.uniform(math.pi/2, 3* math.pi/2)
                    self.state_event_step_rate_xcor = math.cos(new_direction)
                    self.state_event_step_rate_xcor = math.sin(new_direction)
                elif self.state_event_ycor >= deploy_range_y and self.state_event_step_rate_xcor >= 0:
...
class ExperienceReplay(object):
    ...
    def _build_model(self):
        if transfer_learning:
            DRL_json_file = open('DRL_model_experiment/DRL_model_random_direction_benchmark.json', 'r')
            loaded_model_json = DRL_json_file.read()
            DRL_json_file.close()
            model = model_from_json(loaded_model_json)
            # load weights into new model
            model.load_weights("DRL_model_experiment/DRL_model_random_direction_benchmark.h5")
...
if __name__ == "__main__":
    flag_benchmark_topology = 1  # 0: use coordinates of on the saved files; 1: make new coordinates.
    ...
    # DNN parameters.
    hidden_size0 = 100  #600
    hidden_size1 = 100  #300
    hidden_size2 = 150
    max_memory = 2000
    batch_size = 40
    ...
    for a in range(epoch):
        ...
        f = open('DRL_model_convergence_experiment/DRL_results_random_direction_benchmark.txt', 'a+')
        ...
        if a % 25 == 0:
            ...
            with open("DRL_model_experiment/DRL_model_random_direction_benchmark.json", "w") as json_file:
                json_file.write(model_json)
            # serialize weights to HDF5
            exp_replay.model.save_weights("DRL_model_experiment/DRL_model_random_direction_benchmark.h5")
