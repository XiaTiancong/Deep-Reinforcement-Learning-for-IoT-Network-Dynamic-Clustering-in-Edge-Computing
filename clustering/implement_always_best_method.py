...
class Cluster(object, ):
    ...
    def get_state(self, tick):
        ...
        state = state + self.behavior  # here we add the position and behavior sesing
    ...
    def set_events(self):
        if read_AGV:
            ...
            self.df_sampled = self.agv.add_behavior(self.df_sampled, threshold)
            self.df_sampled = self.agv.fit_to_canvas(self.df_sampled)
    ...
    def act_move_events(self, total_tick):
        if read_AGV:
            self.state_event_xcor = self.df_sampled['pos_x'][total_tick]
            self.state_event_ycor = self.df_sampled['pos_y'][total_tick]
            self.behavior = [self.df_sampled['dir'][total_tick]]
            self.event_position = [self.state_event_xcor, self.state_event_ycor]
...
if __name__ == "__main__":
    ...
    Dueling = True
    read_AGV = True
    always_best_experience_method = True
    ...
    if not always_best_experience_method:
        best_method_reward = np.zeros((grid_num_x, grid_num_y))
        best_method_reward.fill(np.nan)

        best_method_state = np.zeros((grid_num_x * grid_num_y, server_number*3))
    ...
    for a in range(epoch):
        ...
        for tick in range(0, game_time):  # In every epoch, we move the event for game_time.
            ...
            if always_best_experience_method:
                ...
                if np.isnan(best_method_state[int(round(env.state_event_xcor//grid_size_x) * grid_num_x + round(env.state_event_ycor//grid_size_y)) ,0]):
                    action = env.act_action(mem_state, tick)

                else:
                    best_method_state_reshaped = best_method_state.reshape((grid_num_x, grid_num_y, -1))
                ...
                if np.isnan(best_method_in_mem):
                    ...
                    dim_transfer = int(round(env.state_event_xcor // grid_size_x) * grid_num_x + round(
            ...
            if flag_gameover_mode == 1:
                if game_over == 1:
                    break
            if total_tick >= len(env.df_sampled['pos_x']):
                total_tick = 0
        ...
        f = open('dqn_results_best_method_mem_test.txt', 'a+')
